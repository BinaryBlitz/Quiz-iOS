...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  if (indexPath.row == 0) {
    ...
    if (self.isCurrent) {
      ...
      if (self.friendRequests) {
        if (self.friendRequests.count > 0) {
          ...
          JSBadgeView *bv = [[JSBadgeView alloc]
              initWithParentView:playerCell.friendsButton
                       alignment:JSBadgeViewAlignmentTopRight];
          bv.badgeText =
              [NSString stringWithFormat:@"%ld", (unsigned long)
                                                     self.friendRequests.count];
        }
      }
    } else {
    ...
    [playerCell.playerUserpic
        setImage:[QZBCurrentUser sharedInstance].user.userPic];
    ...
    topicCell.topicName.text =
        [NSString stringWithFormat:@"топик %ld", (long)indexPath.row];
    return topicCell;
    // cell = topicCell;
  }
  return cell;
...
- (void)initFriendsWithUser:(id<QZBUserProtocol>)user {
  ...
  if (self.isCurrent) {
    [[QZBServerManager sharedManager]
        GETFriendsRequestsOnSuccess:^(NSArray *friends) {
